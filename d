...
game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Made by Louis&7eleven",
                Text = "Have fun with nova",
                Duration = 5  
            })
...
farmTab:AddToggle({
    Name = "Anti-Fall",  -- Name des Toggles
    Default = false,  -- Standardwert (ausgeschaltet)
    Callback = function(state)
        if state then
            -- Aktiviert den Anti-Fall-Schaden
            ...
            getfenv().nofall = game:GetService("RunService").RenderStepped:Connect(function()
                ...
                if character then
                    ...
                    if humanoidRootPart then
                        -- Überprüft, ob der Spieler fällt (unter -30 Y-Geschwindigkeit)
                        ...
                        if raycastResult and humanoidRootPart.Velocity.Y < -30 then
                            -- Setzt die Fallgeschwindigkeit auf 0, um Fall-Schaden zu verhindern
            ...
            -- Deaktiviert den Anti-Fall-Schaden
...
-- Function to set health to 0 (kill the character)
local function setHealthToZero()
    local character = player.Character
    if character and character:FindFirstChild("Humanoid") then
        local humanoid = character:FindFirstChild("Humanoid")
        humanoid.Health = 0  -- This will set the character's health to 0
    end
end

-- Call the function to kill the character (set health to 0)
setHealthToZero()
...
local function startHovering(direction)
    local character = player.Character
    if character then
        local humanoidRootPart = character:FindFirstChild("HumanoidRootPart")
        local humanoid = character:FindFirstChild("Humanoid")
        ...
        if humanoidRootPart and humanoid then
            ...
            humanoidRootPart.CFrame = humanoidRootPart.CFrame + Vector3.new(0, hoverHeight, 0) + direction.Unit * hoverSpeed
        end
    end
end
            ...
            if otherCharacter then
                ...
                if otherRootPart then
                    ...
                    if distance < 30 then
                        ...
                        local direction = humanoidRootPart.Position - otherRootPart.Position
                        startHovering(direction) 
                        return
                    end
                end
            end
...
local function xray()
    for _, part in ipairs(workspace:GetDescendants()) do
        if part:IsA("BasePart") and not part.Parent:FindFirstChildWhichIsA("Humanoid") and not part.Parent.Parent:FindFirstChildWhichIsA("Humanoid") then
...
local player = game.Players.LocalPlayer -- Lokaler Spieler
local character = player.Character or player.CharacterAdded:Wait() -- Charakter des Spielers
local humanoidRootPart = character:WaitForChild("HumanoidRootPart") -- Hauptteil des Charakters
local rotationSpeed = 5 -- Geschwindigkeit der Rotation (je höher, desto schneller)
local rotationRadius = 3 -- Radius der Rotation um den Zielspieler
local moveSpeed = 0.6 -- Geschwindigkeit der Bewegung
local activationRadius = 10 -- Radius, in dem die Rotation sofort startet

local isRotating = false -- Status der Rotation
local isMoving = false -- Status der Bewegung
local targetPlayer = nil -- Zielspieler
local canMove = false -- Status, ob die Bewegung aktiviert wurde

-- Dienste
local userInputService = game:GetService("UserInputService")
local mouse = player:GetMouse()
local runService = game:GetService("RunService")
local humanoid = character:WaitForChild("Humanoid") -- Der Humanoid des Charakters für Bewegungen

-- Funktion: Spieler auswählen durch Anklicken
mouse.Button1Down:Connect(function()
    if not canMove then
        return -- Wenn das Toggle deaktiviert ist, passiert nichts
    end

    if isRotating or isMoving then
        return -- Wenn bereits rotiert oder bewegt wird, nichts tun
    end

    -- Prüfe, ob das angeklickte Objekt zu einem Spieler gehört
    local target = mouse.Target
    if target and target.Parent then
        local targetCharacter = target.Parent
        local clickedPlayer = game.Players:GetPlayerFromCharacter(targetCharacter)
        if clickedPlayer and clickedPlayer ~= player then
            -- Bewegung starten, um zum Zielspieler zu gehen
            isMoving = true
...
-- Funktion: Bewege den Charakter zum Zielspieler
local function moveToTarget()
    ...
    -- Berechne die Richtung zum Zielspieler
    local direction = (targetPosition - humanoidRootPart.Position).Unit
    local distance = (humanoidRootPart.Position - targetPosition).Magnitude

    -- Bewege den Charakter zum Zielspieler, wenn der Abstand groß genug ist
    if distance > 2 then
        humanoid:MoveTo(targetPosition)
    else
        -- Wenn der Zielspieler erreicht ist, beginne mit der Rotation
        isRotating = true
        isMoving = false
    end
end

-- Funktion: Charakter dreht sich um Zielspieler
local function rotateAroundTarget()
    ...
    -- Berechne den Winkel für die Rotation basierend auf der Zeit
    local angle = tick() * rotationSpeed
    local offsetX = math.cos(angle) * rotationRadius
    local offsetZ = math.sin(angle) * rotationRadius

    -- Setze den neuen CFrame um den Zielspieler
    local newPosition = Vector3.new(targetPosition.X + offsetX, humanoidRootPart.Position.Y, targetPosition.Z + offsetZ)
    humanoidRootPart.CFrame = CFrame.new(newPosition, targetPosition) -- Schau immer zum Zielspieler
end

-- Schleife: Bewegung und Rotation ausführen
runService.RenderStepped:Connect(function()
    -- Leertaste: Rotation abbrechen
    ...
    -- Wenn der Spieler sich zum Ziel bewegt
    if isMoving then
        moveToTarget()
    end

    -- Wenn die Rotation aktiviert ist, drehe den Charakter um das Ziel
    if isRotating then
        rotateAroundTarget()
    end
end)

-- Toggle für die "Easy Arrest" Funktion
MiscTab:AddToggle({
    Name = "Easy Arrest (click on a criminal to easily arrest)",
    Default = false,
    Callback = function(value)
        if value then
            -- Toggle aktiviert: Bewegung wird aktiviert
            canMove = true
            -- Benachrichtigung anzeigen
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Easy Arrest",
                Text = "Click Space to stop ,,Easy Arrest''.",
                Duration = 5  
            })
        else
            -- Toggle deaktiviert: Bewegung wird deaktiviert
            canMove = false
            isRotating = false
            isMoving = false
        end
...
local function addGlowEffect(part, targetColor)
...
local function makeCarRainbow()
    ...
    while autoRainbowToggle do
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("BasePart") and isCarPart(obj) then
                addGlowEffect(obj, rainbowColors[colorIndex])
            end
        end
...
local function toggleRainbowEffect(value)
    autoRainbowToggle = value
    if autoRainbowToggle and not rainbowEffectActive then
        task.spawn(makeCarRainbow)
    end
end
